...
if (!content.includes('import ManageCredentials')) {
    content = content.replace("import BusMap from '../components/BusMap';", "import BusMap from '../components/BusMap';\nimport ManageCredentials from '../components/ManageCredentials';\nimport StudentLookup from '../components/StudentLookup';");
}

// 2. Add extra state variables for reassignment and incidents
if (!content.includes('const [incidents')) {
    content = content.replace("const [buses, setBuses] = useState([]);", "const [buses, setBuses] = useState([]);\n  const [incidents, setIncidents] = useState([]);\n  const [reassignForm, setReassignForm] = useState({ bus_number: '', temp_driver: '', temp_driver_phone: '', temp_driver_bus: '', reason: '', end_date: '' });\n  const [reassignSubmitting, setReassignSubmitting] = useState(false);");
}

// 3. Update loadData to conditionally fetch and handle errors gracefully
const loadDataMatch = content.match(/const loadData = useCallback\(async \(\) => \{[\s\S]*?\}, \[auth\]\);/);
if (loadDataMatch) {
    ...
    try {
      const [dash, studs, att, busList, incs, reass] = await Promise.all([
        ...
        ['admin', 'accountant'].includes(auth.role) ? api.getStudents(auth).catch(() => []) : Promise.resolve([]),
    ...
    content = content.replace(loadDataMatch[0], newLoadData);
}

// 4. Add handleReassignSubmit function
if (!content.includes('handleReassignSubmit')) {
    const useEffectMatch = content.match(/useEffect\(\(\) => \{[\s\S]*?\}, \[loadData\]\);/);
    if (useEffectMatch) {
        content = content.replace(useEffectMatch[0], useEffectMatch[0] + `
  ...
  const handleReassignSubmit = async (e) => {
    ...
    try {
      await api.scan({
        student_id: 'SYSTEM',
        bus_number: reassignForm.temp_driver_bus,
        stop_name: 'REASSIGNMENT',
        driver_name: reassignForm.temp_driver,
        is_reassignment: true,
        reassignment_data: reassignForm,
        password: auth.token // Assuming this uses the same auth mechanism or was previously open
      });
...
// 5. Replace the tabs array mapping
const tabsMappingMatch = content.match(/\{\['overview', 'attendance', 'students', 'buses'\]\.map\(\(tab\) => \(/);
if (tabsMappingMatch) {
    ...
    content = content.replace(tabsMappingMatch[0], tabsReplacement);
}

// 6. Insert new tabs content
if (!content.includes("activeTab === 'manage_credentials'")) {
    const lastTabMatch = content.match(/\{activeTab === 'buses' && \([\s\S]*?\}\)[\s\S]*?<\/div>\n      \)\}/);
    if (lastTabMatch) {
        const newTabs = `
...
`;
        content = content.replace(/\{activeTab === 'buses' && \([\s\S]*?\}\)[\s\S]*?<\/div>/, lastTabMatch[0] + newTabs);
    }
}
